...
export function normalizeText(text: string): string {
    ...
    cleaned = cleaned
        ...
        .replace(/0(?=\s*km)/gi, "O") // Fix 0 km read as O km issue if any (usually opposite) -> actually usually 0 is correct for numbers
        .replace(/\bJl\s0\b/gi, "Jl.") // "Jl 0" -> "Jl."
        ;
    ...
    const structuralMap: [RegExp, string][] = [
        ...
        [/\b(u|un)\.?\s/gi, "Unit "],
        [/\b(rt)\.?\s?0*(\d+)/gi, "RT $2"], // Fix RT 01 -> RT 1 (optional preference, let's keep 01 usually? User said "RT 01" is SAFE, so maybe keep it)
        [/\b(rt)\.?\s/gi, "RT "],
        [/\b(rw)\.?\s/gi, "RW "],
    ];
    ...
    // We expand these to make it easier for AI to classify them as "Notes"
